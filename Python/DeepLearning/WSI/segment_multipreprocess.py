...
save_file = '/media/zzr/Data/skin_xml/mask_result/temp/'
...
def PredictThread(nProcs, startCol, endCol, row_cords, stride, image_dilation, mag_factor, slide, mask, proc):
    caffe.set_mode_gpu()
    caffe.set_device(0)
    ...
    ProcessBatchSize = batchsize // nProcs
    ...
    for i in col_cords:
        for j in row_cords:
            if int(image_dilation[stride * j // mag_factor, stride * i // mag_factor]) != 0:
                ...
                temp += 1
                #
                if temp == ProcessBatchSize or (i == col_cords[-1] and j == row_cords[-1]):
                    net.forward()
                    for idx in range(ProcessBatchSize):
                    ...
                    jj = []
                    temp = 0
                    print 'Proc %d, %d / %d' % (proc, i, col_cords[-1])

    np.save(save_file + 'Result_' + str(proc) + '.npy', mask)


def overallMask():
    for idx, data in enumerate(os.listdir(save_file)):
        if idx == 0:
            mask = np.load(os.path.join(save_file, data))
        else:
            mask += np.load(os.path.join(save_file, data))

    np.save(save_file + 'Result_Overall.npy', mask)
    ...
    for name1 in img_WSI_all:
        ...
        for bounding_box in bounding_boxes:
            ...
            for proc in xrange(nProcs):
                startCol = ColPerCore * proc
                endCol = min(ColPerCore * (proc + 1), len(col_cords))
                ProcessPointer[proc] = Process(target=PredictThread, args=(
                        nProcs, startCol, endCol, row_cords, stride, image_dilation,
                        mag_factor, slide, mask, proc))
