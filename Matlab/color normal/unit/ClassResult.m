function [CR tongji]= ClassResult(label, data, model, type)
% by faruto
...
for i = 1:numel(model.Label)
    ...
    tongji(1,i)=p;%%%%%%%%%%%%此处修改
    tongji(2,i)=CR.accuracy(i+1)*100;%%%%%%%%%%%%%%%%%%%此处修改
    str = ...
        sprintf('第 %d 类分类准确率 = %g%% (%d/%d)',model.Label(i),CR.accuracy(i+1)*100,p,p+n);
    disp(str);
end
tongji(3,i)=CR.accuracy(1)*100;%%%%%%%%%%%%%%%%%%此处修改
%%
if type == 1
...
if numel(model.Label) == 2 && type == 1
    ...
    for i = 1:size(model.sv_coef,1)
        CR.alpha(i) = CR.w(i)/label(CR.SVlocation(i));  %%%%%%%%%%%%%%%此处修改
    end
end