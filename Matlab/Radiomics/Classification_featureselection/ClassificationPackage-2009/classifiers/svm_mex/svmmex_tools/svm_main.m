...
%%
[FPR,TPR,T,AUC,OPTROCPT,~,~] = perfcurve(testing_labels,prob_estimates(:, 1),1);  % calculate AUC. 'perfcurve' can also calculate sens, spec etc. to plot the ROC curve.
[TP, FN] = perfcurve(testing_labels,prob_estimates(:, 1),1,'xCrit','TP','yCrit','FN');
[FP, TN] = perfcurve(testing_labels,prob_estimates(:, 1),1,'xCrit','FP','yCrit','TN');
optim_idx = find(FPR == OPTROCPT(1) & TPR == OPTROCPT(2));
outputs.tp = TP(optim_idx);
outputs.fn = FN(optim_idx);
outputs.fp = FP(optim_idx);
outputs.tn = TN(optim_idx);
%%
