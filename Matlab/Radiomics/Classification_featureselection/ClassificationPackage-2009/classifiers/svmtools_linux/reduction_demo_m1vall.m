...
dimensions = [1]; %choose dimensions
methods = [10]; %choose methods
fs = 1; %feature selection? fs = 0; %off fs = 1; %on
if fs == 0
    load svm_stats_11_1_07
else
    load svm_stats_11_2_07
end
...
%    krange = trunk; %run for smaller range of k values for Isomap/LLE
        ...
        for method = methods
            ...
            if method ==5 | method ==6 | method == 8| method == 9
                ...
                for k = krange
                    ...
                    if dataset == 17
                        [tp, tn, fp, fn, prediction] = run_demo_multiclass(temp, labels);
                    else
    ...
    if fs == 0
        save('svm_stats_11_1_07.mat','acc_table','spec_table','sens_table','time_table','allkacc','allksens','allkspec','allktime','avg_acc_table','avg_spec_table','avg_sens_table','avg_time_table','k_table','precision_acc_table','precision_time_table');fprintf('Saved.');
    else
        save('svm_stats_2_10_08.mat','acc_table','spec_table','sens_table','time_table','allkacc','allksens','allkspec','allktime','avg_acc_table','avg_spec_table','avg_sens_table','avg_time_table','k_table','precision_acc_table','precision_time_table');fprintf('Saved.');
    end
    %%--------------------------end dataset--------------------------------%%
end
toc
