...
warning off
datasets = [1 3 4 8 10 12 14 15 18 21]; %choose datasets
dimensions = [2]; %choose dimensions
methods = [6]; %choose methods
fs = 0; %feature selection? fs = 0; %off fs = 1; %on
...
for dataset = datasets
    ...
    %krange = 2; %run for smaller range of k values for Isomap/LLE
    fprintf('%s\n', titlestring);
        ...
        for method = methods
            ...
            if method ==5 | method ==6 | method == 8| method == 9
                ...
                for k = krange
                    ...
                    if dataset == 17
                        [tp, tn, fp, fn] = run_demo_multiclass(temp, labels);
                    else
...
%     if fs == 0
%         save('svm_stats_11_11_07.mat','acc_table','spec_table','sens_table','time_table','allkacc','allksens','allkspec','allktime','avg_acc_table','avg_spec_table','avg_sens_table','avg_time_table','k_table','precision_acc_table','precision_time_table');fprintf('Saved.');
%     else
