...
if numel(unique(training_labels))~=3, error('Only 3 unique labels supported at this time.'); end
...
%     stats.ppv = PPV(optim_idx);
    ...
    stats.num_correct = nnz(testing_labels == stats.decision);
    stats.acc = stats.num_correct/length(stats.decision);
