    ...
    short Saved;
{
	...
	ForEach(i, 0, MaxItem)
	{
	    ...
	    if ( PrunedClass != RealClass )  {
	      //printf("\n\n ITEM %3d   TRUE %3d   PREDICTED %3d\n", 
	      //         i+1,RealClass+1,PrunedClass+1);
	      printf("\n\n ITEM %3d\n",i);

	       Category_IPA(Item[i], Pruned[t]);
	       totalErr++;
	    }
	    /* IPA[022503] - end */
	...
	printf("\n IPAError %lf IPATree %d\n", 100.0*PrunedErrors/(MaxItem+1.0), TreeSize(Pruned[t]));
    }
        printf("\n YannisError %3d\n", totalErr);
