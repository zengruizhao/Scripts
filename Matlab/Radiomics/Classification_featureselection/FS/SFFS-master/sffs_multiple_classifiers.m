function cols = sffs_multiple_classifiers(trainlabels, traindata, testlabels, testdata)
...
%% add feature
    for col = 1:length(featureaddresults)
        ...
        newcols(end+1) = col; 
        try
            featureaddresults(col) = lda_classifier(traindata, trainlabels, testdata, testlabels, newcols);
        catch
            continue
        end
    end
    ...
    while updated
        ...
        if (length(cols) > 1) % need atleast one vector or won't be able to predict
            ...
            for col = 1:length(featuresubtractresults)
                ...
                fewercols(col) = []; 
                try
                    featuresubtractresults(col) = lda_classifier(traindata, trainlabels, testdata, testlabels, fewercols);
                catch
                    continue
                end
            end
...
function featurea_results = svm_classifier(traindata, trainlabels, testdata, testlabels, newcols)
        ...
        featurea_results = correctprct;
end

function featurea_results = lda_classifier(traindata, trainlabels, testdata, testlabels, newcols)
        data = traindata(:,newcols);
        MdlLinear = fitcdiscr(data,trainlabels);
        [label, score] = predict(MdlLinear,testdata(:,newcols));
